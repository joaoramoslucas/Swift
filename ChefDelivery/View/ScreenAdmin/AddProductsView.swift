...
struct AddProductsView: View {
    ...
    var body: some View {
        VStack {
            ...
            // Campos
            inputField(title: "Nome do Produto", placeholder: "Digite o nome", text: $productName)
                .padding(.top, 32)

            inputField(title: "Descrição do Produto", placeholder: "Digite a descrição", text: $productDescription)
                .padding(.top, 15)

            inputField(title: "Valor", placeholder: "R$", text: $productPrice, keyboard: .decimalPad)
                .padding(.top, 15)
                .onChange(of: productPrice) { newValue in
                    productPrice = formatCurrency(newValue)
                }
            ...
            Button {
                addProduct()
                dismiss() // Voltar para a lista após adicionar
            } label: {
...
#Preview {
    NavigationView {
        AddProductsView(viewModel: ProductViewModel())
    }
}
