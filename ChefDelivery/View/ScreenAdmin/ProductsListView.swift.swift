...
struct ProductsListView: View {
    ...
    var body: some View {
            ...
            if viewModel.products.isEmpty {
                VStack {
                    Image(systemName: "cart.badge.plus")
                        .resizable().scaledToFit().frame(width: 100, height: 100).foregroundColor(.gray.opacity(0.5))
                    Text("Nenhum produto adicionado.").font(.subheadline).foregroundColor(.gray)
                }
            } else {
                List {
                    ForEach(viewModel.products, id: \.id) { product in
                        HStack(spacing: 16) {
                            if let image = product.image {
                                image
                                    .resizable().scaledToFill().frame(width: 60, height: 60).clipShape(RoundedRectangle(cornerRadius: 12))
                            } else {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                Text(product.price) // product.price é String aqui
        ...
        .navigationTitle("Meus Produtos").navigationBarTitleDisplayMode(.inline)
    }
}
