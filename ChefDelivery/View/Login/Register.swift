...
struct RegisterView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                Image("logo") // Imagem local
                    .resizable().frame(width: 300, height: 300).padding(.bottom, 30)
                ...
                VStack(spacing: 20) {
                    TextField("Email", text: $email)
                        ...
                        .padding(.horizontal, 20).foregroundColor(.black).keyboardType(.emailAddress).autocapitalization(.none)
                    ...
                    Button(action: { register() }) {
                        Text("Criar Conta").font(.headline).foregroundColor(.white).padding()
                            .frame(width: 200, height: 50).background(Color.orange).cornerRadius(10)
                    }.padding()

                    Button("Já tenho uma conta") { dismiss() }.foregroundColor(.blue)
                }.padding(.bottom)
            }
        }
    }
    ...
    private func register() {
        ...
        FirebaseAuth.Auth.auth().createUser(withEmail: email, password: password) { result, error in
            if let error = error { errorMessage = error.localizedDescription; print("Erro ao criar conta:", error.localizedDescription) }
            else { print("Conta criada com sucesso!"); dismiss() }
        }
    }
}
