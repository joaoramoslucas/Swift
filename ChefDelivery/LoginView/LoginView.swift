...
struct LoginView: View {
    ...
    var body: some View {
        ZStack { // Camada de sobreposição para gerenciar a exibição de elementos
            Color.white.ignoresSafeArea() // Define o fundo branco, ignorando as áreas seguras
            VStack { // Organiza os elementos verticalmente
                ...
                VStack(spacing: 20) { // Organiza elementos verticalmente com espaçamento
                    ...
                    TextField("Email", text: $email) // Campo de texto que liga a variável email
                        ...
                        .background(Color.gray.opacity(0.2)) // Cor de fundo cinza claro
                    ...
                    HStack { // Organiza os botões horizontalmente
                        socialButton(title: "Google", icon: "globe", color: .red) // Botão para login com Google
                        socialButton(title: "Facebook", icon: "f.square", color: .blue) // Botão para login com Facebook
                        socialButton(title: "Telefone", icon: "phone", color: .green) // Botão para login com telefone
                    }
                }
                .padding(.bottom, 50) // Adiciona espaçamento na parte inferior do contêiner
    ...
    private func socialButton(title: String, icon: String, color: Color) -> some View {
        Button(action: {
            ...
            .foregroundColor(.white) // Define a cor do texto como branca
            .padding() // Adiciona espaçamento interno ao botão
            .frame(width: 120, height: 50) // Define a largura e altura do botão
